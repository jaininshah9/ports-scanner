...
def ports(ports):
    ...
    try:
        ...
        if re_list.match(ports):
            try:
                return [port for port in ast.literal_eval(ports)
                        if port in ip_range]
            except:
            ...
            if start in ip_range and stop in ip_range and stop > start:
                ...
                raise argparse.ArgumentTypeError("port must be 0-65535 \
                                    and 2nd ip should be greater than 1st ip")
        else:
...
def main():
    ...
    parser.add_argument("ports", type=ports,
                        help="Single port or list of ports (eg, 22,111)\
                        or range(eg: 22:113)")
